...
module.exports = new Command({
    ...
    async run(bot, message, args, db) {
        ...
        if(user.id === developper) return message.reply("Vous ne pouvez pas kick mon créateur");
        ...
        await message.reply(`${user.tag} a été expulsé par ${message.user === undefined ? message.author.tag : message.user.tag} pour la raison ${reason} avec succès !`).then(async msg => {
            ...
            await message.guild.members.cache.get(user.id).kick(`${reason} (Explusé par ${message.user === undefined ? message.author.tag : message.user.tag})`)
