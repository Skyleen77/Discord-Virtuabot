    ...
    async run(bot, message, args, db) {
        ...
        try {
            ...
            message.channel.bulkDelete(number).catch(async err => {
                ...
                try {
                    await message.reply(`${message.author === undefined ? message.user : message.author} a supprimé \`${msg.size}\` messages`);
                } catch (err) {
                    await message.channel.send(`${message.author === undefined ? message.user : message.author} a supprimé \`${msg.size}\` messages`)
                        .then(async mess => setTimeout(async () => {mess.delete()}, 5000));
                }
